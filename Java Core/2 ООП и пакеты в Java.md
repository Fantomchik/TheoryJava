...
- [12. Что такое метод и чем отличается от функции, возвращаемое значение, аргументы метода?]()
- [13. Что такое final? Что может быть final? Как они работают?]()
- [14. Как реализована неизменность String?]()
- [15. Как реализовать свой Immutable тип данных?]()
- [16. Что такое ключевое слово static?  Что может быть static?]()
- [17. Могут ли нестатические методы перегрузить статические?]()
- [18. Что такое пакет? Как создать пакет?]()
- [19. Что такое конструкторы? Конструктор по-умолчанию?]()
- [20. Расскажи, что такое this и super? Когда мы обязаны использовать this и super?]()
- [21. Класс Object и все методы кроме wait, notify, notifyAll]()
- [22. Зачем нужен метод finalize?]()
- [23. Из-за чего происходят коллизии?]()
- [24. В чём разница между instanceOf и getClass?]()
- [25. Перечислите методы именно класса Enum?]()
- [26. Что такое интерфейс? Когда использовать интерфейс?]()
- [27. Можно ли создать поля в интерфейсе?]()
- [28. Есть ли в Java множественное наследование? Зачем реализовывать интерфейс интерфейсу?]()
- [29. Можно ли создавать статик методы в интерфейсах?]()
- [30. Что такое абстрактный метод?]()
- [31. Что такое абстрактный класс? Чем отличается от обычного?]()
...
1.  **Инкапсуляция** — скрытие внутренней реализации объекта и предоставление доступа к его данным только через методы (геттеры/сеттеры). Это повышает безопасность и упрощает поддержку кода.
2.  **Наследование** — возможность создавать новые классы на основе существующих, что позволяет повторно использовать код и минимизировать его дублирование.
3.  **Полиморфизм** — возможность использовать объекты разных классов через общий интерфейс, что облегчает расширяемость программы.
4.  **Абстракция** — выделение ключевых характеристик объекта и игнорирование ненужных деталей. Это упрощает восприятие системы в целом.
...
1.  **Инкапсуляция**  
    Скрытие внутренней реализации объекта и предоставление доступа к его данным только через публичные методы (геттеры и сеттеры). Это помогает предотвратить прямое изменение состояния объекта, а также улучшает безопасность и поддержку кода.
    
2.  **Наследование**  
    Возможность создавать новые классы на основе уже существующих, что позволяет повторно использовать код и расширять функциональность без необходимости его переписывать.
    
3.  **Полиморфизм**  
    Способность объектов разных классов реагировать на одинаковые сообщения (вызовы методов) по-разному. Полиморфизм позволяет работать с объектами через общий интерфейс, что делает программу более гибкой и расширяемой.
...
*   Класс `String` в Java **неизменяем**: как только строка создается, ее содержимое не может быть изменено.
