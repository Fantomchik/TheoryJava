...
- [Интерфейсы в механизме обратного вызова](#интерфейсы-в-механизме-обратного-вызова-вопросы)
...
## Что такое отношения IS-A и HAS-A [вопросы](#вопросы)
...
### Более детализированная классификация взаимодействий
...
- - **Агрегация** - Отношение когда один объект является частью другого, но **может существовать без него**.
- - **Композиция** - Более тесная связь, когда объект вообще **не может принадлежать другому объекту**.
...
## Могут ли в языке Java у абстрактного класса быть конструкторы? [вопросы](#вопросы)
...
## Могут ли абстрактные классы в языке Java реализовывать интерфейсы? Должны ли они реализовывать все методы? [вопросы](#вопросы)
...
## Может ли абстрактный класс быть final? [вопросы](#вопросы)
...
## Могут ли у абстрактного класса в языке Java быть статические методы? [вопросы](#вопросы)
...
## Обязательно ли в абстрактном классе должны быть абстрактные методы? [вопросы](#вопросы)
...
## Каковы различия между абстрактным классом и интерфейсом в Java? [вопросы](#вопросы)
...
## Может ли абстрактный класс в Java содержать метод main? [вопросы](#вопросы)
...
## Интерфейсы в механизме обратного вызова [вопросы](#вопросы)

### Интерфейсы в механизме обратного вызова (Callback)

**Обратный вызов (callback)** — это механизм, при котором один объект передаёт управление другому через интерфейс. Это используется для асинхронных операций, событий и настройки поведения.

#### 🔹 Как это работает?

1.  **Определяется интерфейс** с методом обратного вызова.
2.  **Класс-исполнитель** вызывает этот метод, когда нужно.
3.  **Класс-пользователь** реализует интерфейс и передаёт свою реализацию в исполнитель.

* * *

### 🔹 Пример: Callback для уведомления о завершении задачи
...
// Класс, выполняющий задачу и вызывающий callback
class Worker {
...
// Класс, использующий Worker
public class Main {
    public static void main(String[] args) {
        Worker worker = new Worker();

        // Передаём анонимный класс как callback
...
```
Работаем...
Получен результат: Работа завершена!
```

* * *

### 🔹 Современный вариант с лямбда-выражением

Так как `Callback` — это функциональный интерфейс, можно использовать **лямбду**:

```java
worker.doWork(result -> System.out.println("Получен результат: " + result));
```
...
✔ **Слушатели событий (Event Listeners)** – кнопки, таймеры  
✔ **Асинхронные операции** – загрузка данных, сетевые запросы  
✔ **Шаблон "Наблюдатель" (Observer Pattern)**

**Вывод:** Интерфейсы в callback'ах позволяют передавать поведение как аргумент, делая код гибким и удобным для расширения. 🚀
